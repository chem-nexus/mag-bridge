...
> [!NOTE]
> In packaged builds, Electron starts backend on `127.0.0.1` using a dynamic free port (not fixed `8000`).
> Frontend receives this runtime URL via `window.appConfig` exposed by preload.
