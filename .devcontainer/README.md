...
- **Purpose:** Uses the container's isolated tools to download heavy project requirements (e.g., `npm install`, `uv venv`). _Note: In cloud environments like GitHub Codespaces, pre-builds pause and cache the state at this exact step._
...
**6. Service Wake-up (`postStartCommand`)** _[Optional]_

- **Trigger:** Runs **inside** the container _every_ time it wakes up from a stopped state (e.g., reopening a laptop).
